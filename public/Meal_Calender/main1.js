...
// Get the query parameters from the URL
    ...
    if (success === 'true') {
        // Show a popup or perform any action indicating successful login
        showPopupAndRedirect(name,id) ;
    }
    ...
    document.getElementById('redirectToPractice').addEventListener('click', (e) => {
        e.preventDefault();
        window.location.href = `../Meal_Calender/practice.html?name=${encodeURIComponent(name)}&id=${encodeURIComponent(id)}`;
    });
    document.getElementById('redirectToPractice1').addEventListener('click', (e) => {
        e.preventDefault();
        window.location.href = `../Meal_Calender/practice1.html?name=${encodeURIComponent(name)}&id=${encodeURIComponent(id)}`;
    });
    document.getElementById('redirectToMenu').addEventListener('click', (e) => {
        e.preventDefault();
        window.location.href = `../Menu/menu.html?name=${encodeURIComponent(name)}&id=${encodeURIComponent(id)}`;
    });
    document.getElementById('redirectToManager').addEventListener('click', (e) => {
        e.preventDefault();
        window.location.href = `../Current_Manager/currentm.html?name=${encodeURIComponent(name)}&id=${encodeURIComponent(id)}`;
    });
    document.getElementById('redirectToDiningFee').addEventListener('click', (e) => {
        e.preventDefault();
        window.location.href = `../Dining_Fee/dining_fee.html?name=${encodeURIComponent(name)}&id=${encodeURIComponent(id)}`;
    });
    document.getElementById('redirectToLateFee').addEventListener('click', (e) => {
        e.preventDefault();
        window.location.href = `../Late_Fee/table.html?name=${encodeURIComponent(name)}&id=${encodeURIComponent(id)}`;
    });
...
function showPopupAndRedirect(name,id) {
    Swal.fire({
        title: 'Login Successful!',
        text: `Welcome, ${name}!`,
        icon: 'success',
        confirmButtonText: 'OK',
        confirmButtonColor: 'hsl(24, 62%, 50%)' // Customize confirm button color if needed
    }).then((result) => {
        if (result.isConfirmed || result.isDismissed) {
            // Redirect to practice.html after the popup is closed
            window.location.href = `../Meal_Calender/practice.html?name=${encodeURIComponent(name)}&id=${encodeURIComponent(id)}`;
        }
    });
...
async function checkFeastDate() {
    try {
        ...
        const lunchFeast = feasts.find(feast => {
            ...
            return feastDate >= today && feast.meals[0].toLowerCase() === 'lunch';
        });
